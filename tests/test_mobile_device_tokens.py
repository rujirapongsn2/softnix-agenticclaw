...
def test_mobile_registration_requires_pairing_token(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir(parents=True)
    config_path = tmp_path / "config.json"
    config_path.write_text(
    ...
    try:
        service.register_mobile_client("prod", "mob-1", None, "Tester")
    except PermissionError as exc:
        assert "Pairing token is required" in str(exc)
    else:
